#Auther : Kamlesh Kumar Rangi
#Dated: 14 March 2018
#Title: Script to install MongoDB and Create sharding with support for 3.6 on ubuntu 16.04

echo "Installing MongoDB 3.6 Latest one"
...
mongod --replSet configserver --logpath "cfg-a.log" --dbpath /data/config/config-a --port 57040 --fork --configsvr 
mongod --replSet configserver --logpath "cfg-b.log" --dbpath /data/config/config-b --port 57041 --fork --configsvr 
mongod --replSet configserver --logpath "cfg-c.log" --dbpath /data/config/config-c --port 57042 --fork --configsvr 
...
mongos --logpath "mongos-1.log" --configdb configserver/localhost:57040,localhost:57041,localhost:57042 --fork
...
db.adminCommand({enableSharding: "test0"})
db.adminCommand({shardCollection: "test0.test_id", key: {test_id:1}});
EOF

echo "Congratulations you created mongodb sharding on local"
